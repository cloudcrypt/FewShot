...
class OmniglotLoader:
    ...
    def _convert_path_list_to_images_and_labels(self, path_list, is_one_shot_task):
        ...
        labels = to_var(torch.zeros((number_of_pairs, 1)))
    ...
    def get_train_batch(self):
        ...
        for index in selected_characters_indexes:
            ...
            image_path = os.path.join(
                self.dataset_path, self.processed_folder, 'images_background', current_alphabet, current_character)
    ...
    def get_one_shot_batch(self, support_set_size, is_validation):
        ...
        image_path = os.path.join(
            self.dataset_path, self.processed_folder, image_folder_name, current_alphabet, current_character)
    ...
    def one_shot_test(self, model, support_set_size, number_of_tasks_per_alphabet,
        ...
        for alphabet in alphabets:
            ...
            for _ in range(number_of_tasks_per_alphabet):
                ...
                probabilities = model(to_torch_to_var(images[0]), to_torch_to_var(images[1]))
                ...
                if torch.argmax(probabilities) == 0 and probabilities.std() > 0.01:
                    accuracy = 1.0
                else:
                    accuracy = 0.0
