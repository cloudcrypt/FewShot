...
class PoseDatasetNShot:
    ...
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz):
        ...
        if not os.path.isfile(os.path.join(root, 'omniglot.npy')):
            ...
            self.x = PoseDataset(root, download=True,
                              transform=transforms.Compose([lambda x: Image.open(x).convert('L'),
                                                            lambda x: x.resize((imgsz, imgsz)),
                                                            lambda x: np.reshape(x, (imgsz, imgsz, 1)),
                                                            lambda x: np.transpose(x, [2, 0, 1]),
        ...
        self.k_shot = 1#k_shot  # k shot
        self.k_query = 1#k_query  # k query
        assert (k_shot + k_query) <=2
        ...
        for sample in range(10):  # num of episodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                for j, cur_class in enumerate(selected_cls):
                    ...
                    x_spt.append(data_pack[cur_class][0])#selected_img[:self.k_shot]])
                    x_qry.append(data_pack[cur_class][1])#selected_img[self.k_shot:]])
...
if __name__ == '__main__':
    ...
    db = PoseDatasetNShot('db/omniglot', batchsz=20, n_way=5, k_shot=5, k_query=15, imgsz=64)
